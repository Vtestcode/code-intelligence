          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            <section className="rounded-[8px] border border-[var(--border)] bg-[var(--panel-muted)] p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex overflow-hidden rounded-[6px] border border-[var(--border)] bg-[var(--bg-base)]">
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold transition ${authMode === 'signin' ? 'bg-[var(--button-primary)] text-white' : 'text-[var(--text-secondary)]'}`}
                  >
                    Sign In
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold transition ${authMode === 'signup' ? 'bg-[var(--button-primary)] text-white' : 'text-[var(--text-secondary)]'}`}
                  >
                    Create Account
                  </button>
                </div>
              ...
              <h2 className="mt-3 text-[1.4rem] font-semibold text-white">
                {authMode === 'signin' ? 'Email and password' : 'Create a JWT account'}
              </h2>
              <p className="mt-3 text-sm leading-[1.5] text-[var(--text-secondary)]">
                {authMode === 'signin'
                  ? 'Sign in directly with your account and receive a JWT-backed session cookie.'
                  : 'Create an account stored in Postgres and sign in with JWT immediately.'}
              </p>
              <div className="mt-5 space-y-3">
                ...
                <button
                  ...
                  className="w-full rounded-[6px] border border-transparent bg-[var(--button-primary)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[var(--button-primary-hover)] disabled:cursor-not-allowed disabled:opacity-60"
                >
            ...
            <section className="rounded-[8px] border border-[var(--border)] bg-[var(--panel-muted)] p-5">
              ...
              <h2 className="mt-3 text-[1.4rem] font-semibold text-white">Use your account</h2>
              <p className="mt-3 text-sm leading-[1.5] text-[var(--text-secondary)]">
                Best if you want a persistent identity for user-scoped repository sessions.
              </p>
              <div className="mt-5">
                {googleClientId ? (
            ...
            <section className="rounded-[8px] border border-[var(--border)] bg-[var(--panel-muted)] p-5">
              ...
              <h2 className="mt-3 text-[1.4rem] font-semibold text-white">Continue without an account</h2>
              <p className="mt-3 text-sm leading-[1.5] text-[var(--text-secondary)]">
                Start immediately with a guest session. You can optionally add a display name.
              </p>
              <div className="mt-5 space-y-3">
                <input
                  value={guestName}
